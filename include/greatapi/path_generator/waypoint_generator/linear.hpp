...
namespace greatapi {
  struct Lines : public WaypointGenerator{
    ...
    int get_segement_upper_index(double PC){
      return std::min((int)ceil(PC*params.size()),(int)params.size()-1);
    }
    ...
    coord computePC(std::vector<coord> target, double PC){
      ...
      distance distscalefac = (PC - (upper_seg-1)/params.size())/params.size();
      return coord(
    ...
    SRAD computeheading(double percCompl){
      ...
      coord vector(params[upper_seg-1], params[upper_seg]);
